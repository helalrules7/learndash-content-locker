<?php
require_once LDCL_PLUGIN_DIR . 'includes/class-ldcl-serial.php';
require_once LDCL_PLUGIN_DIR . 'admin/class-ldcl-admin.php';
require_once LDCL_PLUGIN_DIR . 'public/class-ldcl-public.php';

?>
<div class="wrap ldcl-admin-page">
    <div class="ldcl-contribution">
        <h2><?php _e('Support the developer', 'learndash-content-locker'); ?></h2>
        <p><?php _e('If you find this plugin helpful, please consider supporting me:', 'learndash-content-locker'); ?>
        </p>
        <div class="ldcl-contribution-links">
            <a href="https://buymeacoffee.com/ahmedhelal" target="_blank" class="ldcl-btn ldcl-btn-bmc">
                <i class="fa fa-coffee"></i> <?php _e(' Buy Me a Coffee', 'learndash-content-locker'); ?>
            </a>
            <br> <br>
            <a href="https://ahmedhelal.dev" target="_blank" class="ldcl-btn ldcl-btn-website">
                <i class="fa fa-globe"> <?php _e(' Visit My Website', 'learndash-content-locker'); ?></i>
            </a>
            <br> <br>
            <a href="https://github.com/helalrules" target="_blank" class="ldcl-btn ldcl-btn-github">
                <i class="fa fa-git"></i> <?php _e(' GitHub', 'learndash-content-locker'); ?>
            </a>
        </div>
    </div>

</div>